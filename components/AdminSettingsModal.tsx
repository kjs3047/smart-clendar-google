...
export const AdminSettingsModal: React.FC<AdminSettingsModalProps> = ({ isOpen, onClose, categories, onUpdateCategories, taskTemplates, onUpdateTaskTemplates }) => {
        ...
        <div className="p-4 border rounded-lg">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <button onClick={handleAddCategory} className="p-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90">
              ...
              <div className="pl-6 space-y-2">
                {category.subCategories.map(sub => (
                    <div key={sub.id} className="flex justify-between items-center text-sm">
                        <span>- {sub.name}</span>
                        <button onClick={() => handleDeleteSubCategory(category.id, sub.id)} className="text-gray-400 hover:text-gray-600">
                           <X className="w-4 h-4"/>
                        </button>
                    </div>
                ))}
                <div className="flex gap-2 pt-2">
                    ...
                    />
                    <button onClick={() => handleAddSubCategory(category.id)} className="p-1.5 bg-secondary text-secondary-foreground rounded-md hover:bg-secondary/80">
                    ...
                    <div key={sub.id} className="pl-6 mt-2">
                       ...
                       <div className="pl-2 space-y-1">
                        {(taskTemplates[sub.id] || []).map(template => (
                          <div key={template.id} className="flex justify-between items-center text-sm text-gray-800">
                            ...
                            <button onClick={() => handleDeleteTaskTemplate(sub.id, template.id)} className="text-red-400 hover:text-red-600">
                      ...
                      <div className="flex gap-2 pt-2 pl-2">
                        <input
                          ...
                          onChange={(e) => setNewTaskTemplateContents({ ...newTaskTemplateContents, [sub.id]: e.target.value })}
                           onKeyDown={(e) => e.key === 'Enter' && handleAddTaskTemplate(sub.id)}
                          placeholder="새 태스크 템플릿"
                          className="flex-grow p-1 border text-sm rounded-md"
                        />
                        <button onClick={() => handleAddTaskTemplate(sub.id)} className="p-1.5 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">
                          <Plus className="h-4 w-4" />
                        </button>
                      </div>
                    </div>
