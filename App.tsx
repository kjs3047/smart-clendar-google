...
import * as api from './server/api_axios';
import { BACKEND_URL } from './server/api_axios';
...
const App: React.FC = () => {
  ...
  const handleUpdateKanbanTasks = useCallback(async (newTasks: Task[]) => {
    if (selectedEvent) {
      try {
    ...
    <div className="flex flex-col h-screen bg-gray-50 font-sans">
      {authError && (
        <div className="bg-red-500 text-white text-center p-2">
            {authError}
            <button onClick={() => setAuthError(null)} className="ml-4 font-bold">X</button>
        </div>
      )}
      <header className="flex items-center justify-between p-4 bg-white border-b">
        <div className="flex items-center gap-4">
          <h1 className="text-xl md:text-2xl font-bold text-gray-800">Smart Calendar</h1>
          <button onClick={handleToday} className="px-3 py-1.5 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100">
            Today
          </button>
          <div className="flex items-center">
            <button onClick={handlePrevMonth} className="p-1.5 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-full"><ChevronLeft/></button>
            ...
            <button onClick={handleNextMonth} className="p-1.5 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-full"><ChevronRight/></button>
          </div>
        </div>
        <div className="flex items-center gap-4">
            <button onClick={handleOpenAddEventModal} className="hidden md:flex items-center gap-2 px-3 py-1.5 text-sm font-semibold text-white bg-primary rounded-md hover:bg-primary/90">
                <Plus className="w-4 h-4" />
                일정 추가
            </button>
            <button onClick={handleOpenSettingsModal} className="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-full">
            ...
            {currentUser ? (
                 <>
                    <div className="flex items-center gap-2">
                     ...
                     <button onClick={handleLogout} className="px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-md hover:bg-gray-100">
                        로그아웃
                    </button>
                </>
            ) : (
      ...
      <main className="flex-1 grid grid-cols-7 overflow-y-auto">
        {['일', '월', '화', '수', '목', '금', '토'].map(day => (
          <div key={day} className={`text-center font-medium text-sm py-2 border-b ${day === '일' ? 'text-red-500' : day === '토' ? 'text-blue-500' : 'text-gray-500'}`}>
            {day}
          </div>
        ))}
        {daysInMonth.map((day, index) => {
          ...
          const dayEvents = currentUser ? events
          ...
          return (
            <div
              key={day.toString()}
              className={`relative border-r border-b p-1.5 flex flex-col cursor-pointer transition-colors duration-200 ${isCurrentMonth ? 'bg-white' : 'bg-gray-50'} ${currentUser ? 'hover:bg-blue-50' : 'hover:bg-gray-100'}`}
              onClick={() => handleDayClick(day)}
              style={{minHeight: '120px'}}
            >
              <span className={`text-sm ${isToday ? 'bg-blue-600 text-white rounded-full flex items-center justify-center h-6 w-6 font-bold' : `h-6 w-6 flex items-center justify-center ${dayOfWeek === 0 ? 'text-red-500' : dayOfWeek === 6 ? 'text-blue-500' : isCurrentMonth ? 'text-gray-800' : 'text-gray-400'}`}`}>
                {day.getDate()}
              </span>
