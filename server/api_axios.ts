...
export const apiSaveTaskTemplates = async (templates: { [subCategoryId:string]: TaskTemplate[] }): Promise<{ [subCategoryId: string]: TaskTemplate[] }> => {
    const { data } = await axiosInstance.put<{ [subCategoryId: string]: TaskTemplate[] }>('/v1/task-templates', templates);
    return data;
}