            ...
            DEFAULT_CATEGORIES.map(cat => 
                prisma.category.create({
                    data: {
                        ...
                        userId: userId!,
                        subCategories: {
                            create: cat.subCategories.map(sub => ({ name: sub.name }))
                        }
                    }
                })
            )
...
apiV1Router.put('/categories', async(req: ExpressRequest, res: ExpressResponse) => {
    ...
    try {
      await prisma.$transaction(async (tx) => {
          const existingCatIds = (await tx.category.findMany({ where: { userId: userId! }, select: { id: true }})).map(c => c.id);
          ...
          for (const cat of clientCategories) {
              ...
              if (cat.subCategories) {
                  const existingSubCatIds = (await tx.subCategory.findMany({ where: { categoryId: upsertedCategory.id }, select: { id: true }})).map(sc => sc.id);
      ...
      const updatedCategories = await prisma.category.findMany({ where: { userId: userId! }, include: { subCategories: true } });
...
apiV1Router.put('/events/:eventId/tasks', async (req: ExpressRequest, res: ExpressResponse) => {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        for (const task of clientTasks) {
            ...
            if (comments && comments.length > 0) {
                const existingCommentIds = (await tx.comment.findMany({ where: { taskId: upsertedTask.id }, select: { id: true } })).map(c => c.id);
    ...
    const formattedTasks = finalTasks.map(task => ({
        ...
        content: task.content,
        status: task.status,
        comments: task.comments.map(c => ({ id: c.id, author: c.author.name, content: c.content, createdAt: c.createdAt.toISOString() }))
    }));
    res.json(formattedTasks);
});
